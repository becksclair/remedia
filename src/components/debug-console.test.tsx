...
describe("DebugConsole", () => {
	const createLogEntry = (message: string, level: "info" | "warn" | "error" = "info"): LogEntry => ({
		timestamp: Date.now(),
		source: "test",
		level,
		message
	...
	describe("Search Functionality", () => {
		...
		it("enables Find Next button when search has matches", async () => {
			...
			const findNextButton = screen.getByRole("button", { name: /find next \(1\/1\)/i });
	...
	describe("Find Next Navigation", () => {
		it("cycles through matches when clicking Find Next", async () => {
			...
			const findNextButton = screen.getByRole("button", { name: /find next \(1\/3\)/i });
