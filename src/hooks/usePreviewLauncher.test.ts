...
import {createMockMediaItem, createMockRowSelection, renderWithProviders, waitFor,} from '@/test/test-utils';
...
function PreviewHarness({
  ...
  const {preview} =
      usePreviewLauncher({mediaList, rowSelection, notificationPermission});
...
describe('usePreviewLauncher', () => {
  ...
  it('validates when no media rows are selected', async () => {
    const validationSpy =
        spyOn(ErrorHandlers, 'validation').mockImplementation(() => undefined);
  ...
  it('creates preview windows and sends notifications when permitted',
     async () => {
       const createWindowSpy = spyOn(mockTauriApi.window, 'createWindow');
       const notificationSpy =
           spyOn(mockTauriApi.notification, 'sendNotification');
  ...
  it('reports system errors when window creation fails', async () => {
    ...
    const createWindowSpy =
        spyOn(mockTauriApi.window, 'createWindow').mockImplementation(() => {
          throw error;
        });
    const systemSpy =
        spyOn(ErrorHandlers, 'system').mockImplementation(() => undefined);
