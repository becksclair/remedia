...
function createLogEntry(message: string, level: "info" | "warn" | "error" = "info", mediaIdx?: number): LogEntry {
	return {
		timestamp: Date.now(),
		source: "test",
		level,
...
describe("findLogMatches", () => {
	...
	it("finds single match", () => {
		const logs = [createLogEntry("test message"), createLogEntry("another message"), createLogEntry("final message")];
	...
	it("performs case-insensitive search", () => {
		const logs = [createLogEntry("ERROR Message"), createLogEntry("Error detected"), createLogEntry("no match")];
		const matches = findLogMatches(logs, "error");
